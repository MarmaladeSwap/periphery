...
library MrmOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IMrmPair(pair).price0CumulativeLast();
        price1Cumulative = IMrmPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IMrmPair(pair).getReserves();
